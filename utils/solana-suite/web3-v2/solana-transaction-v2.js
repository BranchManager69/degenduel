// utils/solana-suite/web3-v2/solana-transaction-v2.js

/*
 * This file provides utilities for creating and sending transactions using
 * @solana/transaction-messages and @solana/transactions v2.1
...
export async function transferSOL(connection, fromKeypair, toAddress, amount) {
  try {
    ...
    // Try to create a transfer instruction using helpers first, with fallback to manual construction
    let transferInstruction;
    try {
      // PRIMARY APPROACH: Use SystemProgram helper (cleaner, more reliable)
      transferInstruction = {
      ...
      // FALLBACK APPROACH: Manual instruction construction (more direct)
      logApi.warn('Falling back to manual transfer instruction construction', {
        error: error.message
      });
...
export async function transferToken(
  ...
  try {
    ...
    // Try to create a token transfer instruction using helpers first, with fallback to manual construction
    let transferInstruction;
    try {
      // PRIMARY APPROACH: Use SPL Token helper (cleaner, more reliable)
      // Import createTransferInstruction dynamically to avoid circular dependencies
      const { createTransferInstruction } = await import('@solana/spl-token');
      
      // Use the imported createTransferInstruction to create the instruction data
      const splInstruction = createTransferInstruction(
        fromTokenPubkey,
        toTokenPubkey,
      ...
      // Convert the SPL instruction to our format
      transferInstruction = {
        programId: tokenProgramId,
        accounts: splInstruction.keys.map(key => ({
          pubkey: key.pubkey.toBase58(),
          isSigner: key.isSigner,
          isWritable: key.isWritable
        })),
        data: splInstruction.data
      };
    } catch (error) {
      // FALLBACK APPROACH: Manual instruction construction (more direct)
      logApi.warn('Falling back to manual token transfer instruction construction', {
        error: error.message
      });
...
export async function estimateSOLTransferFee(connection, fromPubkey, toPubkey) {
  try {
    ...
    // System Program ID in base58 format
    const systemProgramId = SystemProgram.programId.toBase58();
    
    // Create a System Program transfer instruction for 1 SOL
    const transferInstructionData = SystemProgram.transfer({
      fromPubkey,
      toPubkey,
      lamports: LAMPORTS_PER_SOL, // Use 1 SOL as a placeholder
    }).data;
    
    // Create the transfer instruction in the format expected by transaction-messages
