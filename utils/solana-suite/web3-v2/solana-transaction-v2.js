 ...
 * @solana/web3.js v2.x and @solana/kit v2.1.0.
 */
...
export async function transferSOL(connection, fromKeypair, toAddress, amount) {
  try {
    ...
    // Create sendAndConfirmTransaction function using factory
...
export async function estimateSOLTransferFee(connection, fromPubkey, toPubkey) {
  try {
    ...
    const estimateComputeUnits = getComputeUnitEstimateForTransactionMessageFactory({
      rpc: connection
    });
    
    // Get compute units estimate from the factory function
    const computeUnitsEstimate = await estimateComputeUnits(message);
