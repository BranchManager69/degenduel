...
The `DEV_LOGIN_SECRET` is a shared secret that is defined in the `.env` file. The default value is `[REDACTED]`.
...
- `DEV_LOGIN_SECRET` - Secret for dev login (default: `[REDACTED]`)
